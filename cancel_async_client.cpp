...
int main(int argc, char *argv[]) {
  ...
  while (!brpc::IsAskedToQuit()) {
    ...
    // ChannelOptions.timeout_ms is timeout in milliseconds for all RPCs via the
    // Channel, Controller.set_timeout_ms() overrides value for one RPC
    cntl->set_timeout_ms(5000);
    const brpc::CallId id = cntl->call_id();
